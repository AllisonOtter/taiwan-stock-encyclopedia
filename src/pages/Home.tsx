...
import { 
  Cpu, Monitor, Sun, Wifi, ShoppingCart, Cloud, Battery, 
  Box, Coffee, Layers, Scissors, Settings, Zap, FlaskConical, Wine, FileText, Shield, Circle, Car, HardHat, 
  Landmark, Activity, Ship, Map, Store, Bike, Home, 
  Leaf, CloudLightning, Flame, Briefcase, LayoutGrid,
  TrendingUp, TrendingDown, ArrowRight, Search
} from 'lucide-react';
import { sectors, macroCategories, type Sector } from '../data/categories';
import '../index.css';
...
const HomePage: React.FC = () => {
  ...
  const filteredSectors = activeMacro === '全部' 
    ? sectors 
    : sectors.filter(s => s.macroCategory === activeMacro);
    ...
    <div className="container" style={{ paddingTop: '4rem', paddingBottom: '4rem' }}>
      ...
      >
        ...
        <p style={{ fontSize: '1.25rem', color: 'var(--text-secondary)', maxWidth: '800px', margin: '0 auto', lineHeight: 1.8 }}>
          探索所有上市上櫃公司的深度解析，涵蓋企業歷史、國際供應鏈地位、地緣政治影響及即時營運狀況。為專業投資者打造的數據驅動決策平台。
        </p>
        ...
        <div style={{ marginTop: '2.5rem', display: 'flex', justifyContent: 'center', gap: '1rem' }}>
          <div className="glass-panel" style={{ display: 'flex', alignItems: 'center', width: '100%', maxWidth: '500px', padding: '0.5rem 1rem', borderRadius: '99px' }}>
            ...
            <input 
              type="text" 
              placeholder="搜尋股票代號或公司名稱 (例如: 2330)..." 
      ...
      {/* Macro Categories Filter */}
      <div style={{ marginBottom: '3rem' }}>
        <h2 style={{ fontSize: '2rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
          <div style={{ width: '6px', height: '32px', background: 'var(--accent-color)', borderRadius: '4px' }}></div>
          產業分類總覽
        </h2>
        
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem' }}>
          {['全部', ...macroCategories].map(macro => (
            <button
              key={macro}
              ...
              style={{
                ...
                borderRadius: '99px',
                border: activeMacro === macro ? '1px solid rgba(59, 130, 246, 0.5)' : '1px solid var(--glass-border)',
                background: activeMacro === macro ? 'rgba(59, 130, 246, 0.2)' : 'rgba(255,255,255,0.03)',
                color: activeMacro === macro ? '#fff' : 'var(--text-secondary)',
                fontWeight: activeMacro === macro ? 600 : 500,
                cursor: 'pointer',
                transition: 'all 0.2s'
              }}
            >
          ...
          {filteredSectors.map((sector: Sector) => {
            ...
            return (
              <motion.div key={sector.id} variants={itemVariants} className="glass-panel" style={{ padding: '2rem', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%' }}>
                ...
                <div style={{ position: 'absolute', top: 0, right: 0, width: '150px', height: '150px', background: 'var(--accent-glow)', filter: 'blur(80px)', opacity: 0.15, zIndex: 0 }}></div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      ...
                      <div>
                        ...
                        <span style={{ fontSize: '0.8rem', color: '#60a5fa', background: 'rgba(59, 130, 246, 0.1)', padding: '2px 6px', borderRadius: '4px' }}>{sector.macroCategory}</span>
                      </div>
                    </div>
                  </div>
